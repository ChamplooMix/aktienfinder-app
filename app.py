...
if st.session_state.view == "Übersicht":
    ...
    for t in tickers:
        ...
        price = inf.get("regularMarketPrice", 0)
        change = df["Change"].iloc[-1] if not df.empty else 0
        pe_ratio = inf.get("trailingPE", "n/a")
        high_52 = inf.get("fiftyTwoWeekHigh", "n/a")
        low_52 = inf.get("fiftyTwoWeekLow", "n/a")

        with st.expander(f"{t} — {market_cap:,} USD MarketCap", expanded=False):
            c1, c2 = st.columns(2)
            c1.write(f"**Kurs:** {price:.2f} {inf.get('currency','')}")
            c2.write(f"**Δ heute:** {change:.2f}%")
            c3, c4 = st.columns(2)
            c3.write(f"**P/E:** {pe_ratio}")
            c4.write(f"**52w H/L:** {high_52} / {low_52}")
...
# Detail-View
    ...
    metrics = [
        ...
        (
            ...
            if inf.get("earningsQuarterlyGrowth")
            else "n/a",
        ),
    ]
